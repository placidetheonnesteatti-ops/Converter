...
def _run_to_tex(run) -> str:
    text = escape_latex(run.text or "")
    if not text:
        return ""
    if run.bold:
        text = r"\textbf{" + text + "}"
...
def _paragraph_runs(p, preserve_page_breaks: bool = True) -> str:
    chunks: list[str] = []
    for run in p.runs:
        chunks.append(_run_to_tex(run))
        for br in run._r.xpath('.//w:br'):
            typ = br.get(qn('w:type'))
            if typ == 'page' and preserve_page_breaks:
                chunks.append(r"\newpage")
            elif typ == 'column':
    ...
    return "".join(chunks) or escape_latex(p.text)
...
def _paragraph_tex(p, image_map: dict[str, str], options: ConversionOptions) -> tuple[str, int]:
    ...
    text = _paragraph_runs(p, options.preserve_page_breaks).strip()
    image_count = 0
    ...
    if not text:
        return "", image_count
    if heading:
        cmd = "section" if heading == 1 else "sub" * (heading - 1) + "section"
        return f"\\{cmd}{{{text}}}\n\n", image_count
    align = _alignment(p)
    if align == "center":
...
def _table_tex(table, index: int, options: ConversionOptions) -> str:
    ...
    if not cols:
        return ""
    env = "longtable" if len(rows) >= 25 else "tabularx"
    if env == "tabularx":
...
def _section_geometry(doc) -> tuple[str, str]:
    """Approximate the first Word section's page size and margins in LaTeX geometry."""
    if not doc.sections:
        return "a4paper", "top=2.5cm,bottom=2.5cm,left=2.5cm,right=2.5cm"
    ...
    try:
        ...
        page = f"a4paper" if abs(width_cm-21.0) < 0.2 and abs(height_cm-29.7) < 0.2 else f"paperwidth={width_cm:.2f}cm,paperheight={height_cm:.2f}cm"
        if orientation == "landscape":
            page += ",landscape"
        margins = (
            f"top={section.top_margin.cm:.2f}cm,bottom={section.bottom_margin.cm:.2f}cm,"
            f"left={section.left_margin.cm:.2f}cm,right={section.right_margin.cm:.2f}cm"
        )
        return page, margins
    except Exception:
        return "a4paper", "top=2.5cm,bottom=2.5cm,left=2.5cm,right=2.5cm"
...
def convert_docx(source: Path, options: ConversionOptions) -> ConversionResult:
    ...
    try:
        doc = Document(source)
        image_map, image_count = _extract_images(source, project)
        body_parts: list[str] = []
        table_count = 0
        list_stack: list[str] = []
            ...
            if child.tag == qn("w:p"):
                ...
                if para is not None:
                    ...
                    if info:
                        ...
                        body_parts.append(r"\item " + _paragraph_runs(para, options.preserve_page_breaks).strip() + "\n")
                    else:
                        while list_stack:
                            body_parts.append(r"\end{" + list_stack.pop() + "}\n")
        ...
        paper, geometry = _section_geometry(doc)
        tex = wrap_document("".join(body_parts), paper=paper, geometry=geometry, header=header, footer=footer)
